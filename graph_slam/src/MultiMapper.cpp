...
*/	}else 
	if(mState == ST_MAPPING)
	{
		...
		if(success)
		{
			...
			tf::Pose map_in_robot(tf::createQuaternionFromYaw(corrected_pose.GetHeading()), btVector3(corrected_pose.GetX(), corrected_pose.GetY(), 0.0));
	...
	catch(karto::Exception e1)
	{
		if(mOtherLasers.find(scan->robot_id) == mOtherLasers.end())
		{
			try
			{
				...
				mOtherLasers.insert( pair<int,karto::LaserRangeFinderPtr>(scan->robot_id,laser) );
